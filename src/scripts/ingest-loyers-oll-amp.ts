 ...
 * Ingestion OLL AMP (Aix-Marseille-Provence) aggregate → LoyerCommune N1bis pour 13055.
 *
 * Source  : Base_OP_2024_L1300.zip — https://www.observatoires-des-loyers.org/datagouv/2024/Base_OP_2024_L1300.zip
 *           Attendu sur VPS : /tmp/probe-loyers/Base_OP_2024_L1300.zip
 * Format  : CSV semicolon, encodage Latin-1 (28 colonnes, même format que L7500/L6900)
 *
 * Filtre exact (probe step 2 validé) :
 *   Zone_calcul                    = "" (vide)
 *   Zone_complementaire            = "" (vide)
 *   Type_habitat                   = ""
 ...
 *
 * → Ligne unique : loyer_median=13,0 | loyer_1_quartile=10,9 | loyer_3_quartile=15,6 | nombre_observations=29778
 *
 * Note : le périmètre L1300 couvre l'agglomération AMP entière (Marseille est la commune
 * principale, représentant la majorité des observations). Meilleure approximation
 * disponible via OLL pour 13055 au niveau N1bis.
 *
 * IDEMPOTENCE : ON CONFLICT (commune_id) DO UPDATE — re-run sans danger.
 *
 * Usage :
 *   npm run ingest:oll-amp
 *   npm run ingest:oll-amp -- --zip=/custom/path.zip
 */
...
const DEFAULT_ZIP = '/tmp/probe-loyers/Base_OP_2024_L1300.zip';
...
// Witnesses probe step 2 — fail = STOP avant upsert
const PROBE_LOYER  = 13.0;
const PROBE_Q1     = 10.9;
const PROBE_Q3     = 15.6;
const PROBE_NB_OBS = 29778;
...
function parseCsv(buf: Buffer): OllRow {
  ...
  for (const rawLine of lines) {
    ...
    // Filtre aggregate L1300 : toutes les 9 colonnes de sous-population vides
    if (!isAllEmpty(cols,
      colMap.zoneCalc, colMap.zoneCompl, colMap.typeHabitat,
  ...
  console.log(`  → ${lineCount} lignes lues | ${matches.length} match(es) agrégat AMP toutes sous-pop`);

  if (matches.length === 0) {
    throw new Error(
      `Aucune ligne agrégat AMP (toutes sous-populations vides).\n` +
...
async function main(): Promise<void> {
  const t0 = Date.now();
  console.log('=== OLL AMP (L1300) → 13055 (N1bis) — ingestion ===');
  console.log(`ZIP : ${ZIP_PATH}`);

  console.log('\n[1/4] Extraction ZIP (Base_OP_2024_L1300.csv)...');
  ...
  console.log('\n[4/4] Upsert LoyerCommune 13055 (idempotent)...');
